...
class UserPurchaseFlowTests(TestCase):
    ...
    def test_new_user_can_register_and_purchase(self):
        ...
        # Assert registration succeeded (status 201) + extract token
        ...
        # Assert products endpoint returns 200 + at least one product
    ...
    def test_user_can_view_their_orders(self):
        ...
        self.client.credentials(HTTP_AUTHORIZATION=f'Token {token}')
        create_order_response = self.client.post('/api/orders/', {
        ...
        # Serializer may return product name in different shapes depending on implementation
        if 'product' in order and isinstance(order['product'], dict):
            self.assertEqual(order['product'].get('name'), self.product.name)
        elif 'product_name' in order:
            self.assertEqual(order['product_name'], self.product.name)
        else:
            # fallback if API only returns product id
            if 'product_id' in order:
                self.assertEqual(order['product_id'], self.product.id)
            elif 'product' in order:
                self.assertEqual(order['product'], self.product.id)
            else:
                self.fail("Order response does not contain product information (name or id).")
...
class PaymentValidationTests(TestCase):
    ...
    def test_valid_card_creates_paid_order(self):
        ...
        # Assert status is 201 Created + paid
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_declined_card_returns_payment_error(self):
        ...
        # Assert status is 402 Payment Required + error exists
